...
def poreto_noniid(dataset, num_users):
    ...
    while key:
        try:
            ...
            print(total)
            sample_client = [math.ceil(total * dis[i])
                             for i in range(num_users)]
    ...
    if sum(random_shard_size) > num_shards:
        ...
        for i in range(num_users):
            ...
            rand_set = set(np.random.choice(
                idx_shard, shard_size, replace=False))
            idx_shard = list(set(idx_shard) - rand_set)
            for rand in rand_set:
                dict_users[i] = np.concatenate(
                    (dict_users[i], idxs[rand *
...
def sta(client_dict, train_dataset, num_users):
    ...
    for client in range(num_users):
        tmp = []
        for i in range(10):
            tmp.append(sum(train_dataset[j][1] ==
                       i for j in client_dict[client]))
        rs.append(tmp)
...
if __name__ == "__main__":
    ...
    index = 1
    save_dataset_idx(
        client_dict, f"10client/FashionMNIST-noniid-mnist_noniid_sample_{index}.json"
    )
    df = sta(client_dict, train_dataset, num_users)
